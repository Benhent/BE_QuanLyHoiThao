        ...
        try
        {
            ...
            if (_jwt.ValidateToken(token, out ClaimsPrincipal? claims))
            {
                ...
                using (var reader = new StreamReader(Request.Body))
                {
                    ...
                    command.Parameters.AddWithValue("@FirstName", jsonObject["first_name"]?.ToString() ?? (object)DBNull.Value);
                    command.Parameters.AddWithValue("@LastName", jsonObject["last_name"]?.ToString() ?? (object)DBNull.Value);
                    ...
                    command.Parameters.AddWithValue("@InstitutionID", jsonObject["institution_id"]?.ToObject<int?>() ?? (object)DBNull.Value);
    ...
    public async Task<ActionResult> GetFullName(int id)
    {
        try
        {
            ...
            if (_jwt.ValidateToken(token, out ClaimsPrincipal? claims))
            {
                ...
                command.Connection = connection;
                command.CommandType = CommandType.StoredProcedure;
                command.CommandText = "spAuthor_GetFullName";
